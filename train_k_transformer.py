...
for epoch in range(1, n_epochs+1):
    ...
    with torch.no_grad():
        for X_obs, Y_fut, _ in tqdm(val_loader, desc=f"Epoch {epoch} [val]"):
            ...
            # Forward pass — K-aware
            x1_pred = model(x_t, t_sample, X_obs)            # (B, K, D)
            
            # Best-of-K loss
            gt_mse = (x1_pred - x1[:, None]).pow(2).mean(dim=-1)  # (B, K)
            k_star = gt_mse.argmin(dim=1)
            x1_star = x1_pred[torch.arange(B, device=device), k_star]
            
            raw_mse = (x1_star - x1).pow(2).mean(dim=-1)
            L_gt = (raw_mse / (1 - t_sample).clamp(min=1e-2).pow(2)).mean()
            
            # MoD loss
            y_pred = x1_pred.view(B, K, pred_len, 2)
            ...
            # Smoothness loss
            y_all = x1_pred.view(B, K, pred_len, 2)
            vel = y_all[:, :, 1:] - y_all[:, :, :-1]
            acc = vel[:, :, 1:] - vel[:, :, :-1]
            L_smooth = acc.pow(2).mean()
